...
purchase_day = st.sidebar.number_input("Purchase Day", min_value=0,
max_value=6, step=1)
purchase_month = st.sidebar.number_input("Purchase Month", min_value=1,
max_value=12, step=1)
purchase_year = st.sidebar.number_input("Purchase Year", min_value=2016,
max_value=2025, step=1)
product_size = st.sidebar.number_input("Product Size (cm³)", min_value=0.0,
step=1.0)
product_weight = st.sidebar.number_input("Product Weight (g)",
min_value=0.0, step=1.0)
customer_state = st.sidebar.selectbox("Customer State",
label_encoders["customer_state"].classes_)
seller_state = st.sidebar.selectbox("Seller State",
label_encoders["seller_state"].classes_)
shipping_method = st.sidebar.selectbox("Shipping Method",
label_encoders["shipping_method"].classes_)
...
input_data = pd.DataFrame([[purchase_day, purchase_month, purchase_year,
product_size, product_weight,
 customer_state_encoded, seller_state_encoded,
shipping_method_encoded]],
 columns=['purchase_day', 'purchase_month',
'purchase_year', 'product_size',
 'product_weight', 'customer_state',
'seller_state', 'shipping_method'])
